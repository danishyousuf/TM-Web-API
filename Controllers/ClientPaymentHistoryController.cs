...
namespace TMCC.Controllers
{
    ...
    public class ClientPaymentHistoryController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteClientPayment(string paymentId, string deletedBy)
        {
            ...
            return BadRequest(new { message = "Failed to delete payment" });
        }
        [Authorize]
        [HttpDelete("delete-all/{clientId}/{deletedBy}")]
        public async Task<IActionResult> DeleteAllClientPayments(string clientId, string deletedBy)
        {
            var result = await _service.DeleteAllClientPaymentsAsync(clientId, deletedBy);

            if (result > 0)
            {
                return Ok(new { message = "All payments deleted successfully" });
            }

            return BadRequest(new { message = "Failed to delete payments" });
        }
